...
widths = [w for w in sorted(int(x) for x in P["widths"]) if w <= max(available, 1)]
if not widths:
...
R = Result()
R.metric("best_throughput", "Throughput at the widest", top["tokens_per_sec"], "num", "kept", help=f"{top['gpus']} GPUs")
R.metric(f"efficiency_{top['gpus']}", f"Efficiency at {top['gpus']} GPUs", top["efficiency"], "pct", "sky", help="speedup divided by the number of GPUs; 100% would be perfect")
...
R.metric("mfu", "Utilisation per GPU", top["mfu"], "pct", "hold")
R.chart("speedup", "Speedup against GPU count", rows, "gpus", [{"key": "speedup", "label": "Measured", "color": "kept"}, {"key": "ideal", "label": "Perfect scaling", "color": "hold"}], "line", note="The gap between the lines is the all-reduce. A small model has a short step, so the fixed communication cost is a larger share of it.")
R.chart("efficiency", "Efficiency", rows, "gpus", [{"key": "efficiency", "label": "Efficiency", "color": "sky"}], "line", y_domain=[0, 1])
R.chart("memory", "Peak memory per GPU", rows, "gpus", [{"key": "peak_gb", "label": "GB", "color": "raw"}], "bar", note="Data parallelism replicates the model on every GPU, so memory per GPU does not fall as the node widens.")
