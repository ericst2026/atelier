...
try:
    ...
    if not data:
        ...
        for gpus in [g for g in (1, 4) if g <= available]:
            env = dict(os.environ, ATELIER_GPUS=str(gpus))
            progress(15 + 40 * (gpus == 4), f"running the student recipe on {gpus} GPU(s)")
            proc = subprocess.run(["python", str(entry), "--data", data, "--iters", "200"], cwd=str(project), env=env, capture_output=True, text=True, timeout=2400)
            if proc.returncode != 0:
                raise RuntimeError("\n".join((proc.stdout + proc.stderr).splitlines()[-15:]))
            rows.append(json.loads((project / "outputs" / "throughput.json").read_text()))
        test("runs at both widths", len(rows) >= 1, f"completed {len(rows)} run(s)", 20, 20)
    ...
    R.metric("tokens_per_sec", "Throughput", best["tokens_per_sec"], "num", "kept").metric("per_gpu", "Per GPU", per_gpu, "num", "sky")
