...
R.chart("cost", "What each costs", [{"mode": r["mode"], "peak_gb": r["peak_gb"], "seconds": r["seconds"]} for r in runs], "mode", [{"key": "peak_gb", "label": "Peak GB", "color": "raw"}, {"key": "seconds", "label": "Seconds", "color": "hold", "axis": "right"}], "bar", note=None if device == "cuda" else "This run was on CPU: peak GB is GPU memory and was not measured, so it reads 0.")
