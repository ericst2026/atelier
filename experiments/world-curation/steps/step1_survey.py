...
R.metric("chars", "Characters", sum(len(d["text"]) for d in docs), "int", "sky")
R.chart("kinds", "Documents by kind", [{"kind": k, "count": v} for k, v in kinds.most_common()], "kind", [{"key": "count", "label": "Documents"}], "donut", note="Each kind is a different problem. Some are obvious on one axis, some are only visible by comparing documents with each other.")
