...
const KINDS = [
  { id: "loss", title: "Loss", test: (k) => /loss|perplexity|ppl/.test(k) },
  { id: "reward", title: "Reward", test: (k) => /reward/.test(k) },
  { id: "accuracy", title: "Accuracy", test: (k) => /accuracy|(^|_)acc($|_)|solved|pass_rate|win_rate|recall/.test(k) },
  { id: "kl", title: "KL from the starting model", test: (k) => /(^|_)kl($|_)/.test(k) },
  { id: "grad", title: "Gradient norm", test: (k) => /grad_norm|gradient/.test(k) },
  { id: "margin", title: "Margin", test: (k) => /margin/.test(k) },
  { id: "logp", title: "Log-probability", test: (k) => /logp/.test(k) },
  { id: "lr", title: "Learning rate", test: (k) => k === "lr" || /learning_rate/.test(k) },
  { id: "speed", title: "Speed", test: (k) => /per_sec|per_s$|throughput/.test(k) },
];
...
// how a curve is labelled on a chart it shares
const LABELS = {
  loss: "train batch",
  train_loss: "train",
  val_loss: "validation",
  ...
  mean_token_accuracy: "train (token)",
  eval_mean_token_accuracy: "validation (token)",
...
export function buildLiveCharts(live, { xLabel, max = Infinity, timeline = true } = {}) {
  ...
  const xName = xLabel || live?.x_label || "step";
  ...
  for (const [key, pts] of Object.entries(series)) {
    ...
    if (key.includes("__")) {
      ...
      if (kind >= 0) add(KINDS[kind].id, KINDS[kind].title, kind, [key, pts, label]);
      else add(`own-${group}`, titled(words(group)), OWN_RANK, [key, pts, label]);
      continue;
    }
    const rank = KINDS.findIndex((k) => k.test(key.toLowerCase()));
    if (rank >= 0) add(KINDS[rank].id, KINDS[rank].title, rank, [key, pts, labelOf(key, KINDS[rank].id)]);
  ...
  if (timeline && charts.length === 0 && Array.isArray(tl) && tl.length > 1) {
    charts.push({ id: "live-timeline", title: "Progress over time", type: "line", x: "x", x_label: "seconds", series: [{ key: "y", label: "% done", color: "sky" }], data: tl, y_domain: [0, 100] });
  }
